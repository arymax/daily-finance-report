...
def build_market_prompt(
    ...
    if session == "morning":
        market_output_spec = [
            ...
            "",
            "## 八、宏觀漣漪分析",
            "根據今日最顯著的 2-4 個宏觀變數（油價 / 匯率 / 利率 / 地緣政治等），分析其漣漪效應。",
            "每個變數一個 ### 子節，格式如下：",
            "### [Emoji] [變數名稱與方向，例：🛢 WTI $119（↑ 高油價）]",
            "- **一階影響**：最直接受到影響的產業與個股（2-3 點）",
            "- **二階影響**：延伸的供應鏈 / 原料 / 競爭格局變化（2-3 點）",
            "- **關聯標的**：受益標的（↑）與承壓標的（↓）以 · 分隔，重要的加 ⚠️",
            "此節務必輸出，若無顯著宏觀變數請以市場中性為題分析。",
            "",
            ...
            "sector: <產業，例如：半導體、資安、光通訊、AI應用、AI基礎建設、企業軟體、能源、航太國防、航運、電子材料>",
            "reason: <觸發事件 + 為什麼值得研究>",
        ...
        market_output_spec = [
            "---",
            "",
            "# 請嚴格依照以下 7 個章節結構輸出【收盤市場總覽】（Markdown 格式）",
            "",
            "## 一、台股今日大盤表現",
